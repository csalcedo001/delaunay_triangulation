...
public:
    Plot()
    {
        ...
        this->window = glfwCreateWindow(640, 480, "Plot", NULL, NULL);
        if (!this->window)
        {
            glfwTerminate();
            exit(EXIT_FAILURE);
        ...
        framebuffer_size_callback(this->window, this->width, this->height);
    ...
    void init_opengl()
    {
        ...
        glColorPointer(3, GL_FLOAT, sizeof(Vertex), &vertex[0].r); // Pointer to the first color
